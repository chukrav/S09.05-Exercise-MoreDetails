...
//      TODO (21) Implement LoaderManager.LoaderCallbacks<Cursor>
    ...
    private final int ID_DETAIL_LOADER = 44;
...
//      TODO (13) Find each of the TextViews by ID
        mDate = (TextView) findViewById(R.id.tv_days_date);
        mDescription = (TextView) findViewById(R.id.tv_weather_description);
        mHighTemp = (TextView) findViewById(R.id.tv_high_temperature);
        mLowTemp = (TextView) findViewById(R.id.tv_low_temperature);
        mHumidity = (TextView) findViewById(R.id.tv_humidity);
        mWind = (TextView) findViewById(R.id.tv_selected_days_wind);
...
//                mWeatherDisplay.setText(mForecastSummary);
            ...
            if (mUri == null) {
                throw new NullPointerException("Uri for DetailActivity cann't be null");
            }
...
//        return null;
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        long localDateGMT = data.getLong(INDEX_WEATHER_DATE);
        String dateText = SunshineDateUtils.getFriendlyDateString(this,localDateGMT,true);
        mDate.setText(dateText);

        String weatherDiscript = SunshineWeatherUtils.getStringForWeatherCondition(this,INDEX_WEATHER_CONDITION_ID);
        mDescription.setText(weatherDiscript);

        double highTemp = data.getDouble(INDEX_WEATHER_MAX_TEMP);
        String highTString = SunshineWeatherUtils.formatTemperature(this, highTemp);
        mHighTemp.setText(highTString);

        double lowTemp = data.getDouble(INDEX_WEATHER_MIN_TEMP);
        String lowTString = SunshineWeatherUtils.formatTemperature(this, lowTemp);
        mLowTemp.setText(lowTString);

        double Humid = data.getDouble(INDEX_WEATHER_HUMIDITY);
        String humidString = getString(R.string.format_humidity, Humid);
        mHumidity.setText(humidString);

        float windSpeed = data.getFloat(INDEX_WEATHER_WIND_SPEED);
        float windDir = data.getFloat(INDEX_WEATHER_DEGREES);
        String windCondit = SunshineWeatherUtils.getFormattedWind(this, windSpeed, windDir);
        mWind.setText(windCondit);

        float airPressure = data.getFloat(INDEX_WEATHER_PRESSURE);
        String airPressString = getString(R.string.format_pressure, airPressure);
        mPressure.setText(airPressString);

        mForecastSummary = String.format("%s - %s - %s/%s", dateText, weatherDiscript, highTString, lowTString);
